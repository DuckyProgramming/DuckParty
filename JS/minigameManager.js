    ...
    reset(){
        ...
        switch(this.minigame){
            ...
            case 32:
                this.entities={players:[],projectiles:[]}
                for(let a=0,la=this.operation.player.length;a<la;a++){
                    this.entities.players.push(new player(this.layer,this.layer.width/2+100-la*100+a*200,this.layer.height/2,1,22,a,this.operation.player[a]))
                }
                this.control.bound={base:{x:0,y:0},width:this.layer.width,height:this.layer.height,radius:0}
            break
            case 33:
                ...
                this.entities={players:[],projectiles:[]}
                for(let a=0,la=this.operation.player.length;a<la;a++){
                    this.entities.players.push(new player(this.layer,this.layer.width/2+this.layer.height*0.2*(a-la*0.5+0.5),this.layer.height*0.8,3,3,a,this.operation.player[a]))
                    last(this.entities.players).direction.main=180
                    last(this.entities.players).direction.goal=180
                }
                this.control.bound={base:{x:0,y:0},width:this.layer.width,height:this.layer.height,radius:0}
            break
        }
    ...
    display(scene){
        switch(scene){
            case 'minigame':
                this.layer.noStroke()
                switch(this.minigame){
                    case 0: case 10: case 24: case 32:
            ...
            case 'minigame':
                ...
                switch(this.minigame){
                    ...
                    case 32:
                        ...
                        if(!this.result.end){
                            if(this.control.timer%(540-this.operation.player.length*60)==60){
                                let loc=floor(random(0,(this.layer.width+this.layer.height)/this.layer.width))
                                if(loc==0){
                                    this.entities.projectiles.push(new projectile(this.layer,this.layer.width*random(0,1),this.layer.height*(-0.1+floor(random(0,2))*1.2),17,{}))
                                }else{
                                    this.entities.projectiles.push(new projectile(this.layer,this.layer.width*(-0.1+floor(random(0,2))*1.2),this.layer.height*random(0,1),17,{}))
                                }
                            }
                            for(let a=0,la=this.entities.players.length;a<la;a++){
                                if(this.result.score[a]>=10){
                    ...
                    case 33:
                        ...
                        if(!this.result.end){
                            this.control.sender+=1/30
                            if(this.control.sender>0){
                                this.control.sender--
                                if(floor(random(0,2))==0){
                                    this.entities.players.push(new player(this.layer,this.layer.width+100,random(this.layer.height*0.05,this.layer.height*0.5),1,23,-1,-1))
                                    last(this.entities.players).direction.goal=-90
                                }else{
                                    this.entities.players.push(new player(this.layer,-100,random(this.layer.height*0.05,this.layer.height*0.5),1,23,-1,-1))
                                    last(this.entities.players).direction.goal=90
                                }
                            }
                            for(let a=0,la=this.entities.players.length;a<la;a++){
                                if(this.result.score[a]>=25){
                                    this.result.end=true
                                }
                            }
                            if(this.result.end){
